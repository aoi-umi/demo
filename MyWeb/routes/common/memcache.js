...
var memcached = new Memcached(config.memcache.hostname);
exports.set = function (key, value, seconds, cb, usePrefix) {
    if(usePrefix)
        key = config.cachePrefix + '_' + key;
    memcached.set(key, value, seconds, cb);
};

exports.get = function (key, cb, usePrefix) {
    if(usePrefix)
        key = config.cachePrefix + '_' + key;
