...
my.pager.prototype = {
    ...
    gotoPage: function gotoPage(page) {
        ...
        var pagerDom = $('#' + opt.pagerId);
        page = parseInt(page);
        if (isNaN(page) || page <= 0) throw new Error('page must be a int and large than 0');
        pagerDom.empty();
        if (opt.changeHandle) {
            self.pageIndex = page;
            opt.changeHandle(function (t) {
                if (t && t.count) {
                    ...
                    if (page == 1) dom.find('[name=myPagerPrev]').closest('li').addClass('disabled');
                    if (page >= totalPage) dom.find('[name=myPagerNext]').closest('li').addClass('disabled');
